...
def data_split(X, y, method="expanding_window_split", validation_split=0.5, train_ratio=0.8, window_size=0.6,
    ...
    if method == "expanding_window_split":
        """ 시간 순서를 유지하면서 학습 데이터의 크기를 점진적으로 확장하는 방식. 
            미래 데이터를 과거 데이터로 예측하는 상황을 모방하면서, 점진적으로 더 많은 데이터를 학습에 활용한다. """
...
for target_col in target_columns:
    ...
    # 데이터 분리 함수 호출
    method = "expanding_window_split"  # 사용할 데이터 분리 방법
    validation_split = 0.5  # 검증 데이터 비율
    ...
    # train_size, val_size, test_size 정의
