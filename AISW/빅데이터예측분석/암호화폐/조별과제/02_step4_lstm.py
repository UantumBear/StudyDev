...
target_column_index = df.columns.get_loc("LUNC-USD_Close")
...
log_file_path = rf"data/model/train_log_{hp.VER}.txt"
# 7. 모델 학습
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, patience):
    ...
    for epoch in range(epochs):
        ...
        with open(log_file_path, "a") as log_file:
            log_file.write(
                f"{epoch + 1},{train_losses[-1]:.4f},{val_losses[-1]:.4f},{train_mse:.4f},{train_mae:.4f},{val_mse:.4f},{val_mae:.4f}\n")
        ...
        if val_losses[-1] < best_val_loss:
            ...
            model_path = rf'data/model/best_model_{hp.VER}.pth'
    ...
    # 그래프 저장
    plt.savefig(rf'data/model/02_step4_lstm_손실_{hp.VER}.png', dpi=300, bbox_inches='tight')  # 파일로 저장
...
# 모델 불러오기
model = LSTMModel(input_size, hidden_size, num_layers, output_size).to(device)
model.load_state_dict(torch.load(rf'data/model/best_model_{hp.VER}.pth'))
...
plt.grid(True)
plt.tight_layout()
plt.savefig(rf'data/model/02_LUNC_예측_2019_2025_{hp.VER}.png', dpi=300)
plt.show()
