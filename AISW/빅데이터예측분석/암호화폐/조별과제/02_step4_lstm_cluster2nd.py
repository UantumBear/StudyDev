...
file_path = 'data/output/02_cluster_1_for_train.xlsx'
...
# 대안 2)
# 데이터 분할 비율 설정
# val_size = int(len(data_scaled) * 0.15)    # Validation 데이터: 과거 15%
# test_size = int(len(data_scaled) * 0.15)   # Test 데이터: 최신 15%
# train_size = len(data_scaled) - val_size - test_size  # Train 데이터: 나머지 70%
# val_data = data_scaled[:val_size]                # Validation 데이터: 과거 15%
# train_data = data_scaled[val_size:val_size + train_size]  # Train 데이터: 중간 70%
# test_data = data_scaled[val_size + train_size:]  # Test 데이터: 최신 15%
...
target_column_index = df.columns.get_loc("DASH-USD_Close")
...
log_file_path = rf"data/model/train_log_cluster2nd_{hp.VER}.txt"
# 7. 모델 학습
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, epochs, patience):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for X_batch, y_batch in val_loader:
                ...
                # 반복 가능한 형태로 변환 후 저장
                val_preds.extend(np.atleast_1d(outputs.cpu().numpy()))  # 1차원으로 보장
                val_targets.extend(y_batch.cpu().numpy())
        ...
        if val_losses[-1] < best_val_loss:
            ...
            model_path = rf'data/model/best_model_cluster2nd_{hp.VER}.pth'
    ...
    # 그래프 저장
    plt.savefig(rf'data/model/02_step4_lstm_손실_cluster2nd_{hp.VER}.png', dpi=300, bbox_inches='tight')  # 파일로 저장
...
# 모델 불러오기
model = LSTMModel(input_size, hidden_size, num_layers, output_size).to(device)
model.load_state_dict(torch.load(rf'data/model/best_model_cluster2nd_{hp.VER}.pth'))
...
print("Past Predictions (First 10):", past_predictions[:10])
...
DAY = 365
alpha = 0.7  # 가중치
noise_scale = 0.01  # 노이즈 크기
window_size = 3  # 이동 평균 윈도우 크기


for _ in range(DAY):  # 미래 365일 예측
    future_input_tensor = torch.tensor(future_input, dtype=torch.float32).unsqueeze(0).to(device)
    # print(f"Step {_}: Input Tensor: {future_input_tensor.cpu().numpy()}")  ## TODO
    with torch.no_grad():
        future_pred = model(future_input_tensor).cpu().numpy().flatten()
    future_predictions.append(future_pred[0])
    print(f"Step {_}: Predicted value: {future_pred[0]}")

    # 슬라이딩 윈도우 업데이트
    #future_input = np.append(future_input[1:], [[future_pred[0]] + [0] * (future_input.shape[1] - 1)], axis=0) ## TODO
    # 방법 1) 가중 평균을 사용하는 방식
    future_input = np.append(
        future_input[1:],
        [[future_pred[0]] + list(future_input[-1, 1:])],
        axis=0
    )

    # 방법2) 이동 평균과 노이즈 반영
    # recent_predictions = future_predictions[-window_size:] if len(
    #     future_predictions) >= window_size else future_predictions
    # moving_avg = np.mean(recent_predictions) if recent_predictions else future_pred[0]
    # noisy_pred = future_pred[0] + np.random.normal(0, noise_scale)
    #
    # # future_input 업데이트
    # future_input = np.append(
    #     future_input[1:],
    #     [[alpha * noisy_pred + (1 - alpha) * future_input[-1, 0]] + list(future_input[-1, 1:])],
    #     axis=0
    # )
...
future_dates = [past_dates.iloc[-1] + pd.Timedelta(days=i) for i in range(1, DAY+1)]  # 미래 날짜 ## TODO
...
plt.grid(True)
plt.tight_layout()
plt.savefig(rf'data/model/02_DASH_예측_2019_2025_cluster2nd_{hp.VER}.png', dpi=300)
plt.show()
