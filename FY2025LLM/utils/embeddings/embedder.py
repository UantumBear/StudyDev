...
class Embedder:
    def __init__(self, model_path: Union[str, os.PathLike] = f"{conf.PROJECT_ROOT_DIRECTORY}/models/BM-K/KoSimCSE-roberta"):
        ...
        print(f"[INFO] 로컬 모델 로딩 완료: {model_path}")

        # 예시 문장으로 임베딩 차원 확인
        dummy_vector = self.model.encode(["테스트 문장"])
        self.vector_dim = len(dummy_vector[0])
        print(f"[INFO] 모델 임베딩 차원: {self.vector_dim}차원")
    ...
    def embed_texts(self, texts: List[str]) -> List[List[float]]:
        ...
        print(f"[INFO] {len(texts)} 문장 벡터화 시작...")
    ...
    def embed_file(self, file_path: str, encoding: str = "utf-8") -> List[dict]:
        ...
        print(f"[INFO] {len(lines)} 줄의 텍스트 파일을 읽어왔습니다.")
        print(f"[INFO] 문장 임베딩 중...")
